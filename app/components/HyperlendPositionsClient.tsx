...
export default function HyperlendPositionsClient() {
  ...
  const loadPositions = useCallback(async (id: string) => {
    const response = await fetch(`/api/snapshots/${id}/positions/hyperlend`);
    const data = await response.json();
  ...
  return (
    <div className="grid" style={{ gap: 24 }}>
      <section className="panel">
        <h2>HyperLend Supplies</h2>
        {assets.length === 0 ? (
          <p style={{ color: 'var(--muted)' }}>No supplied assets yet.</p>
        ) : (
          <table className="table">
            <thead>
              <tr>
                <th>Asset</th>
                <th className="numeric">Supplied</th>
                <th>Collateral</th>
                <th>Chain</th>
                <th>Wallet</th>
              </tr>
            ...
            <tbody>
              {assets.map((asset) => (
                <tr key={asset.id}>
                  ...
                  <td>{asset.isCollateral ? 'Yes' : 'No'}</td>
      ...
      <section className="panel">
        <h2>HyperLend Borrows</h2>
        {liabilities.length === 0 ? (
          <p style={{ color: 'var(--muted)' }}>No borrow positions yet.</p>
        ) : (
          <table className="table">
            <thead>
              <tr>
